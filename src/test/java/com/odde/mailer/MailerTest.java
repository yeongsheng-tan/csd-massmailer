...
public class MailerTest {
	...
	@Test
	public void sendSingleRecipient() throws AddressException,
			MessagingException {
		myMailer.send(validButDummyEmail);
		assertEquals(1, sentMessages.size());
		assertEquals(new InternetAddress("To@dummy.com"), sentMessages.get(0)
				.getAllRecipients()[0]);
	}

	@Test
	public void sendMultipleRecipients() throws AddressException,
			MessagingException {
		...
		for (int i = 0; i < recipients.length; i++) {
			assertEquals(new InternetAddress(recipients[i]), sentMessages
					.get(i).getAllRecipients()[0]);
		}

	}

	@Test
	public void testSendEmailFromAddress() throws AddressException,
			MessagingException {

		String fromEmail = "fromEmail@To.com";
		validButDummyEmail.setFrom(fromEmail);
		myMailer.send(validButDummyEmail);
		assertEquals(new InternetAddress(fromEmail), sentMessages.get(0)
				.getFrom().clone()[0]);
	}

	@Test
	public void testSendEmailSubject() throws AddressException,
			MessagingException {
	...
	public void testSendEmailWithBlankToAddress() throws AddressException,
			MessagingException {
		validButDummyEmail.clearRecipients();
		validButDummyEmail.addRecipientsFromStringThatIsSemicolonDelimited(" ");
		myMailer.send(validButDummyEmail);
	}
